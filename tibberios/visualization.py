    ...
    def _generate_visualization(self) -> None:
        ...
        fig = px.bar(
            ...
            labels={
                ...
                "unit_price": "Spot Price (SEK)<br><sup>10' Shower Cost (SEK)</sup>",
                "comparison_price": "10' Shower Cost (SEK)",
            },
            # TODO: Allow user to set title
            title=f"Hourly Price of Electricity from {start_time_local} to {self._price_df.start_time.max().strftime(r'%d/%m %H:00')}<br>"
            + r"<sup>The prices inside the chart show the estimated cost of a 10' shower at 40C (4 kWh). "
